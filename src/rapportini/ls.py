...
@click.option('-d', '--data', type=click.DateTime(formats=['%Y-%m-%d', '%d-%m-%Y', '%d-%m']),
              default=None, help='La data per cui va inserito il rapportino')
@click.option('-c', '--count', type=click.IntRange(1, 14, clamp=False),
              help='Quanti giorni mostrare', show_default=False)
@click.option('-f', '--format', 'fmt', type=click.Choice(['short', 'full']),
              help='Quanti dettagli vuoi sui rapportini', default='short', show_default=True)
@click.pass_obj
def ls(repo, username, password, data: datetime.datetime, count: Optional[int], fmt):
    ...
    while count > 0:
        ...
        click.secho(f' {total_hours} ore', fg='bright_red' if has_missing_rapportini else 'bright_cyan', nl=True,
