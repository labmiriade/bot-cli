...
class Repo(object):
    ...
    def user_id(self) -> str:
        ...
        if self.use_cache:
            if (aux := self.cache.get(key)) is not None:
                return aux
        res = self.s.get(f'{self.base_url}/user/get-current-user',
                         auth=self.auth).json()
        user_id = res['userId']
    ...
    def res_id(self) -> str:
        ...
        res_id = self.s.get(f'{self.base_url}/anagrafichecore.tecnico/find-by-user',
                            params=params,
                            auth=self.auth).json()['resId']
        self.cache[key] = res_id
        return res_id

    def add_rapportino(self, job_id, task_id, descrizione: str, note: str, hours: int, minutes: int, date: int,
                       office_id: str, flag_transfert: bool, flag_prepay: bool, flag_pay: bool, flag_extrahour: bool):
        commessa = list(filter(lambda x: str(x['jobId']) == str(job_id), self.commesse))
        if len(commessa) != 1:
            print(f'{commessa=}')
